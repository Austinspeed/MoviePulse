  ...
  return (
    <Link to={`/movie/${movie.id}`} className='group relative rounded-lg bg-white hover:shadow-xl shadow-lg dark:bg-gray-800 transition-all overflow-hidden'>
      <div className='relative aspect-[2/3] overflow-hidden'>
        <img src={posterPath} alt={movie.title} className='w-full h-full object-cover hover:scale-110 transition-all duration-300'/>
      </div>
      <div className="p-4">
        <div className='flex justify-between items-center'>
          <h3 className='font-semibold text-gray-900 dark:text-white line-clamp-1'>{movie.title}</h3>
          <div className='flex items-center bg-red-600 rounded-full px-2 py-1'>
            <RiStarFill className='text-yellow-400 w-4'/>
            <span className='ml-1 text-sm font-medium text-white'>{movie.vote_average? movie. vote_average.toFixed(1) : ''}
            </span>
          </div>
        </div>
      </div>
